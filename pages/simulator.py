...
# ── Session state initialisation (must run before any state access) ───────────
DEFAULTS = {
    "step": 1,
    "project_info": {},
    "features": [],
...
# ── Page header ───────────────────────────────────────────────────────────────
st.markdown(
    '<div class="rp-hero-sm"><div class="rp-hero-sm-title">📊 Project Risk Simulator</div>'
    '<div class="rp-hero-sm-sub">Answer 5 quick steps — get your full Monte Carlo risk report with 10,000 simulations.</div></div>',
    unsafe_allow_html=True,
)

# ── Step progress bar (only for steps 1-5) ───────────────────────────────────
_STEP_NAMES = ["Project Info", "Features", "Context (16 Q)", "Team", "Budget & Run"]
...
if 1 <= _s <= 5:
    ...
    for i, label in enumerate(_STEP_NAMES):
        n = i + 1
        if n < _s:
            _style = "background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;"
            _inner = "✓"
        elif n == _s:
            _style = "background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;box-shadow:0 0 0 3px rgba(99,102,241,.25);"
            _inner = str(n)
        else:
            _style = "background:#e0e7ff;color:#94a3b8;"
            _inner = str(n)
        _circles += (
            ...
            f'justify-content:center;font-size:.78rem;font-weight:700;{_style}">{_inner}</div>'
            f'<div style="font-size:.62rem;color:{"#4f46e5" if n==_s else "#94a3b8"};'
            f'font-weight:{"700" if n==_s else "500"};text-align:center;line-height:1.2;">{label}</div>'
            f'</div>'
        )
    st.markdown(
        f'<div style="background:#fff;border:1px solid #e0e7ff;border-radius:14px;padding:.85rem 1.2rem;margin-bottom:1.3rem;box-shadow:0 1px 8px rgba(79,70,229,.06);">'
        f'<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.3rem;">{_circles}</div>'
        f'<div style="margin-top:.6rem;background:#e0e7ff;border-radius:99px;height:5px;overflow:hidden;">'
        f'<div style="width:{_pct}%;height:100%;background:linear-gradient(90deg,#4f46e5,#7c3aed);border-radius:99px;transition:width .4s;"></div>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
...
# ═══════════════ STEP 6 — Results ════════════════════════════════════════════
    ...
    # ── Result hero banner ────────────────────────────────────────────────────
    c_time   = '#6adf8a' if p_on_time>=70   else ('#ffb74d' if p_on_time>=45   else '#ef9a9a')
    c_budget = '#6adf8a' if p_on_budget>=70 else ('#ffb74d' if p_on_budget>=45 else '#ef9a9a')
    c_succ   = '#6adf8a' if p_success>=70   else ('#ffb74d' if p_success>=45   else '#ef9a9a')
    st.markdown(
        f'<div style="background:linear-gradient(135deg,#07050f 0%,#130b2e 40%,#1c1050 70%,#2d0f7e 100%);border-radius:20px;padding:2rem 2.4rem;margin-bottom:1.4rem;box-shadow:0 20px 60px rgba(7,5,15,.4),0 0 0 1px rgba(139,92,246,.15);">'
        f'<div style="font-size:.78rem;color:#9d8ecf;margin-bottom:.25rem;text-transform:uppercase;letter-spacing:2px;font-weight:700;">Risk Report</div>'
        f'<div style="font-size:1.75rem;font-weight:900;color:#f5f3ff;letter-spacing:-.4px;margin-bottom:1.2rem;">{pname}</div>'
        f'<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;">'
        f'<div style="background:rgba(255,255,255,.06);border:1px solid rgba(139,92,246,.2);border-radius:14px;padding:1.1rem;text-align:center;"><div style="font-size:2.2rem;font-weight:900;color:{c_time};">{p_on_time:.0f}%</div><div style="color:#9d8ecf;font-size:.76rem;margin-top:.3rem;">On-Time</div></div>'
        f'<div style="background:rgba(255,255,255,.06);border:1px solid rgba(139,92,246,.2);border-radius:14px;padding:1.1rem;text-align:center;"><div style="font-size:2.2rem;font-weight:900;color:{c_budget};">{p_on_budget:.0f}%</div><div style="color:#9d8ecf;font-size:.76rem;margin-top:.3rem;">On-Budget</div></div>'
        f'<div style="background:rgba(255,255,255,.06);border:1px solid rgba(139,92,246,.2);border-radius:14px;padding:1.1rem;text-align:center;"><div style="font-size:2.2rem;font-weight:900;color:{c_succ};">{p_success:.0f}%</div><div style="color:#9d8ecf;font-size:.76rem;margin-top:.3rem;">Full Success</div></div>'
        f'<div style="background:rgba(255,255,255,.06);border:1px solid rgba(139,92,246,.2);border-radius:14px;padding:1.1rem;text-align:center;"><div style="font-size:1.5rem;font-weight:900;color:#c4b5fd;">{cx["label"]}</div><div style="color:#9d8ecf;font-size:.76rem;margin-top:.3rem;">Complexity {int(cx["overall"]*100)}/100</div></div>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
